...
algae_checks = Blueprint('algae_checks', __name__)

@algae_checks.route("/algae", methods=["POST"])

def algae(all_dataframes,sql_match_tables,errors_dict,project_code,login_info):
	errorLog("Function - algae")
	message = "Custom Algae: Start checks."
	statusLog("Starting Algae Checks")
	errorLog(message)
	...
	for dataframe in all_dataframes.keys():
		...
		if table_name == "tbl_algae":
			algae = all_dataframes[dataframe]
	...
	try:
		...
		## LOGIC ##
		message = "Starting Algae Logic Checks"
		errorLog(message)
		statusLog(message)
		## END LOGIC CHECKS ##
		...
		list_of_stations = pd.unique(algae['stationcode'])
		...
		message = "Critical Error: Failed to run algae checks"	
		errorLog(message)
		state = 1
