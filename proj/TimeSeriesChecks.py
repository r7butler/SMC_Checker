...
timeseries_checks = Blueprint('timeseries_checks', __name__)

@timeseries_checks.route("/timeseries", methods=["POST"])

def timeseries(all_dataframes,sql_match_tables,errors_dict,project_code,login_info):
	errorLog("Function - timeseries")
	message = "Custom TimeSeries: Start checks."
	statusLog("Starting TimeSeries Checks")
	errorLog(message)
	...
	for dataframe in all_dataframes.keys():
		...
		if table_name == "tbl_timeseriesstations":
			stations = all_dataframes[dataframe]
			stations['tmp_row'] = stations.index
		if table_name == "tbl_timeserieslocations":
			locations = all_dataframes[dataframe]
			locations['tmp_row'] = locations.index
		if table_name == "tbl_timeseriesresults":
			results = all_dataframes[dataframe]
			results['tmp_row'] = results.index
		if table_name == "tbl_timeserieseffortcheck":
			effort = all_dataframes[dataframe]
			effort['tmp_row'] = effort.index
		if table_name == "tbl_timeserieseffortdetail":
			detail = all_dataframes[dataframe]
			detail['tmp_row'] = detail.index
	try:
		## CHECKS ##
		...
		## LOGIC ##
		message = "Starting TimeSeries Logic Checks"
		errorLog(message)
		statusLog(message)
		## END LOGIC CHECKS ##
		...
		message = "Critical Error: Failed to run timeseries checks"	
		errorLog(message)
		state = 1
