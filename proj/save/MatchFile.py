...
def getTableAndColumns(db,dbtype,eng):
	...
	if dbtype == "mysql" or dbtype == "mysql-rest":
		...
		query = eng.execute("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_CATALOG='%s' and TABLE_NAME LIKE '%s'" % (db,"tbl%%"))
	elif dbtype == "azure":
...
def matchColumnsToTable(tab_name,tab,sqlFields,tabCounter):
	...
	statusLog("Start matching excel coumns to tables...")
...
def match():
	...
	try:
		...
		#statusLog("Running basic checks on file before proceeding...")
		if os.stat(inFile).st_size == 0:
			...
			# place excel file into a dataframe - old method no need to use resources
			#errorLog("chunksize=1000")
			try:
				errorLog("Loading excel file into dataframe...")
				statusLog("Loading excel file into dataframe...")
				# on_demand argument allows saving memory and time by loading necessary functions (read sheet names)
				#df = pd.ExcelFile(inFile, on_demand=True)
				df = pd.ExcelFile(inFile, keep_default_na=False, na_values=['NaN'])
				errorLog("Finished reading excel file...")
				#statusLog("Finished reading excel file...")
			except ValueError:
				message = "Critical error: Failed to retrieve excel sheet names."
				#errorLog(message)
				#statusLog(message)
				state = 1
			...
			try:
				#errorLog("Running getTableAndColumns routine")
				#statusLog("Running getTableAndColumns routine")
			...
			if sqlFields != 1:
				...
				for sheet in df_tab_names:
					# name only
					tab_name = sheet
					message = "Working with sheet %s..." % tab_name
					print(message)
					#errorLog(message)
					#statusLog(message)
					# assign actual data in tab to tab
					#tab = df.parse(tab)
					# added code to fix issue: Pandas interprets cell values with NA as NaN #3
					# read_excel is too slow for large files
					#tab = pd.read_excel(inFile, keep_default_na=False, na_values=['NaN'], sheetname = tab)
					#sheet = pd.ExcelFile(inFile, keep_default_na=False, na_values=['NaN'], sheetname = tab)
					tab = df.parse(sheet)
					print("Done...")
					#memory_used = tab.memory_usage(index=True,deep=True).sum()
					#message = "Memory Usage: %s" % memory_used
					#errorLog(message)
					#statusLog(message)
					if tab.empty:
					...
					try:	
						...
						if match_result == True:
							...
							message = "Tab %s matches table %s" % (tab_name,tab)
							errorLog(message)
							statusLog(message)
							#count = count + 1
						else:
							...
							errorLog(message)
							statusLog(message)
							#match_tables = "tab: %s has no matching tables" % tab_name
							match_tables.append(match_fields)
		...
		error = 1
		state = 1
	return jsonify(message=message,errors=error,state=state,table_match=match_tables)
